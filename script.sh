...
echo ""
  ...
  echo ""
  ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -p 22 "${SSH_USER}@${SERVER}" "rm -r ${DEST_DIR}/node_exporter" > /dev/null
  scp -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -P 22 -r "${SOURCE_DIR}"* "${SSH_USER}@${SERVER}:${DEST_DIR}"
  ssh -i ~/.ssh/id_rsa -o StrictHostKeyChecking=no -p 22 "${SSH_USER}@${SERVER}" "cd ${DEST_DIR}/node_exporter && chmod +x ./node_exporter && ./node_exporter" > /dev/null
